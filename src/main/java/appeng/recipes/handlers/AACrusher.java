...
public class AACrusher implements ICraftHandler, IWebsiteSerializer
{
	...
	public void setup( final List<List<IIngredient>> input, final List<List<IIngredient>> output ) throws RecipeError
	{
		...
		throw new RecipeError( "AACrusher must have a single input, and single output." );
	}

	@Override
	public void register( IForgeRegistry<IRecipe> registry ) throws RegistrationError, MissingIngredientError
	{
		IActuallyAdditions aa = Integrations.aa();
		for( final ItemStack is : this.pro_input.getItemStackSet() )
		{
			try
			{
				aa.addCrusherRecipe( is, this.pro_output[0].getItemStack() );
			}
			catch( final RuntimeException err )
			{
				AELog.error( "AA not happy - " + err.getMessage() );
			}
		}
	}
